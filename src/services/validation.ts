...
import { validateTaskAggregate, validateTaskProfile } from "./validation/task.js";
import { validateBreakdownAggregate } from "./validation/breakdown.js";
import { addIssue, nonEmpty } from "./validation/common.js";
export { validateRevisionSnapshot } from "./validation/revisions.js";
export { validateTaskProfile } from "./validation/task.js";
...
export function assertValidTaskProfile(profile: TaskProfile): void {
  const issues = validateTaskProfile(profile);
...
export function validateTask(task: Task, profile?: TaskProfile): readonly ValidationIssue[] {
  const issues = [...validateTaskAggregate(task, profile)];
  if (profile !== undefined) issues.push(...validateTaskProfile(profile));
  return issues;
}

export function assertValidTask(task: Task, profile?: TaskProfile): void {
  const issues = validateTask(task, profile);
  if (issues.length > 0) throw new MilestoneValidationError(issues);
}

export function validateBreakdown(breakdown: Breakdown): readonly ValidationIssue[] {
  return validateBreakdownAggregate(breakdown);
}

export function assertValidBreakdown(breakdown: Breakdown): void {
