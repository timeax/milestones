...
import type { ActorRef, Milestone } from "../../model/domain.js";
import type { MilestoneAction, MilestoneActionSubject } from "../editor-contracts.js";
...
export function equalDomainValue(left: unknown, right: unknown): boolean {
  ...
  if (left !== null && right !== null && typeof left === "object" && typeof right === "object") {
    ...
    return equalDomainValue(leftKeys, rightKeys)
      && leftKeys.every((key) => equalDomainValue(leftRecord[key], rightRecord[key]));
  }
  return false;
}

export function ensureOpen(session: EditorSession): void {
...
export function feature(enabled: boolean, name: string): void {
  invariant(enabled, "FEATURE_DISABLED", `${name} is disabled by the milestone profile`, { feature: name });
}
