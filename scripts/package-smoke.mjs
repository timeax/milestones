...
try {
  ...
  await writeFile(
    ...
    `import { ARTIFACT_PROTOCOL_COMPATIBILITY, MilestoneEditor, MilestoneDocument, MilestoneDocumentBuilder, createMilestoneDocument, createMilestoneDocumentContext } from "@elqora/milestones";\nimport { evaluateAcceptance } from "@elqora/milestones/evaluation";\nimport { createGraphSnapshot } from "@elqora/milestones/graph";\nimport { serializeMilestone } from "@elqora/milestones/serialization";\nimport { validateMilestone } from "@elqora/milestones/validation";\nimport { FixedMilestoneClock } from "@elqora/milestones/testing";\nimport { MilestoneDomainError } from "@elqora/milestones/model";\nimport { migrateMilestoneWire } from "@elqora/milestones/migrations";\nimport { MilestoneDocument as DomMilestoneDocument, MilestoneDocumentBuilder as DomMilestoneDocumentBuilder, createMilestoneDocument as domCreateMilestoneDocument, createMilestoneDocumentContext as domCreateMilestoneDocumentContext } from "@elqora/milestones/dom";\nif ([MilestoneEditor, evaluateAcceptance, createGraphSnapshot, serializeMilestone, validateMilestone, FixedMilestoneClock, MilestoneDomainError, migrateMilestoneWire, MilestoneDocument, MilestoneDocumentBuilder, createMilestoneDocument, createMilestoneDocumentContext, DomMilestoneDocument, DomMilestoneDocumentBuilder, domCreateMilestoneDocument, domCreateMilestoneDocumentContext].some(value => typeof value !== "function") || ARTIFACT_PROTOCOL_COMPATIBILITY !== ">=1.1 <2.0") throw new Error("package import failed");\nif (createMilestoneDocument !== domCreateMilestoneDocument || MilestoneDocument !== DomMilestoneDocument || MilestoneDocumentBuilder !== DomMilestoneDocumentBuilder || createMilestoneDocumentContext !== domCreateMilestoneDocumentContext) throw new Error("DOM subpath export identity mismatch");\ntry { await import("@elqora/milestones/editors/internal/session"); throw new Error("internal import unexpectedly succeeded"); } catch (error) { const expected = error?.code === "ERR_PACKAGE_PATH_NOT_EXPORTED" || String(error).includes("Cannot find module '@elqora/milestones/editors/internal/session'"); if (!expected) throw error; }\ntry { await import("@elqora/milestones/dom/documents/criteria"); throw new Error("internal DOM import unexpectedly succeeded"); } catch (error) { const expected = error?.code === "ERR_PACKAGE_PATH_NOT_EXPORTED" || String(error).includes("Cannot find module '@elqora/milestones/dom/documents/criteria'"); if (!expected) throw error; }\n`,
  );
  ...
  await writeFile(
    ...
    `import { MilestoneEditor, type MilestoneEditorOptions, type Milestone, MilestoneDocument, MilestoneDocumentBuilder, createMilestoneDocument, createMilestoneDocumentContext, type MilestoneDocumentContract } from "@elqora/milestones";\nimport type { MilestoneProfile } from "@elqora/milestones/model";\nimport { evaluateAcceptance } from "@elqora/milestones/evaluation";\nimport { createGraphSnapshot } from "@elqora/milestones/graph";\nimport { serializeMilestone } from "@elqora/milestones/serialization";\nimport { validateMilestone } from "@elqora/milestones/validation";\nimport { FixedMilestoneClock } from "@elqora/milestones/testing";\nimport { migrateMilestoneWire } from "@elqora/milestones/migrations";\nimport { createMilestoneDocument as domCreateMilestoneDocument, type MilestoneDocumentContract as DomMilestoneDocumentContract } from "@elqora/milestones/dom";\ndeclare const milestone: Milestone; declare const profile: MilestoneProfile; declare const options: MilestoneEditorOptions;\nnew MilestoneEditor(milestone, profile, options); void [evaluateAcceptance, createGraphSnapshot, serializeMilestone, validateMilestone, FixedMilestoneClock, migrateMilestoneWire];\ndeclare const doc: MilestoneDocumentContract; declare const domDoc: DomMilestoneDocumentContract;\nconst createdDoc = createMilestoneDocument({ milestone, profile }); const builderDoc = new MilestoneDocumentBuilder(milestone, profile).build(); const subpathDoc = domCreateMilestoneDocument({ milestone, profile });\nvoid [doc, domDoc, createdDoc, builderDoc, subpathDoc, MilestoneDocument, createMilestoneDocumentContext];\n`,
  );
  execFileSync(
    "node",
